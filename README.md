<div align="center">

# Phase Backend
**Custom Fortnite private server — made by Burlone, Modified By Silva**

![Skidded](https://img.shields.io/badge/skidded-yes-7F77DD?style=flat-square)
![arena](https://img.shields.io/badge/arena-enabled-1D9E75?style=flat-square)

</div>

---

> ⭐ Please leave a star — it helps a lot <3
---

### ✨ Highlights

- 🏆 **Arena** — fully working with leaderboard support
- 👥 **Party system** — accept friend requests, join lobbies (not fully done) and more
- 🎮 **All game modes** — Battle Royale, Creative, STW and Live Events enabled
- 🛍️ **Item shop** — bot auto-posts a shop image every time it refreshes
- 📱 (SOON) **Mobile Support** - gives acess for people to use android, but still need to modify on routes/main.js to the manifest of the version that their doing
